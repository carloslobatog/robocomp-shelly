...
InnerViewer::InnerViewer( InnerModelMgr innerModel_, const std::string &name_, uint period_, QObject *parent ) : period(period_)
{
	...
	InnerModel *in = innerModel_.get()->copy();;
...
void InnerViewer::run()
{
	while( true)
	{
		lock();	// locks IMV first and and then its copy if Innermodel
			innerModelViewer->update();   //accesses local InnerModel for reading and the osg scenegraph
			viewer.frame();			
		unlock();  // unlocks Innermodel and then IMV
		usleep(period);
	}
}
...
{	
	...
	if (node == NULL)
		throw QString("InnerViewer::remove node: Can't remove not existing element");

	if (item_ == "root")
		throw QString("InnerViewer::remove node: Can't remove root lement");
...
void InnerViewer::addPlane_ignoreExisting(const QString &item_, const QString &parent_, const QVec &center, const QVec &normal, const QString &texture, const QVec &size)
{
	...
	if (parent == NULL)
		throw QString("InnerViewer::addPlane_ignoreExisting: parent element node doesn't exist");
