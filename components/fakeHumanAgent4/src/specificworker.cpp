...
void SpecificWorker::includeInAGM()
{
	...
	int idx=0;
	while ((personSymbolId = worldModel->getIdentifierByType("person4", idx++)) != -1)
	{
		printf("%d %d\n", idx, personSymbolId);
		if (idx > 4) exit(0);
		if (worldModel->getSymbolByIdentifier(personSymbolId)->getAttribute("imName") == "fakeperson4")
		{
			printf("found %d!!\n", personSymbolId);
			break;
		}
	...
	// Symbolic part
	AGMModelSymbol::SPtr person4 =   newModel->newSymbol("person4");
	personSymbolId = person4->identifier;
	printf("Got personSymbolId: %d\n", personSymbolId);
	person4->setAttribute("imName", "fakeperson4");
	person4->setAttribute("imType", "transform");
	AGMModelSymbol::SPtr personSt = newModel->newSymbol("personSt");
	printf("person %d status %d\n", person4->identifier, personSt->identifier);

	newModel->addEdge(person4, personSt, "hasStatus");
	newModel->addEdge(person4, personSt, "noReach");
	newModel->addEdge(person4, personSt, "person");
	
	newModel->addEdgeByIdentifiers(person4->identifier, 3, "in");
	...
	edgeRTAtrs["tz"] = "1950";
	edgeRTAtrs["rx"] = "0";
	edgeRTAtrs["ry"] = "4.7425";
	edgeRTAtrs["rz"] = "0";
	newModel->addEdgeByIdentifiers(100, person4->identifier, "RT", edgeRTAtrs);
...
void SpecificWorker::move (){
  ...
  RoboCompInnerModelManager::coord3D coordInBase;
		...
		innermodelmanager_proxy->transform("root", "fakeperson4", coordInItem, coordInBase);
		...
		innermodelmanager_proxy->setPoseFromParent("fakeperson4", pose);
