...
void SpecificWorker::includeInRCIS()
{
	...
	{	
		...
		pose.ry =3.14;
		pose.rz = 0;
		innermodelmanager_proxy->addTransform("fakeperson3", "static", "root", pose);
		...
		mesh.scaleX = mesh.scaleY = mesh.scaleZ = 8;
		mesh.render = 0;
		//mesh.meshPath = "/home/robocomp/robocomp/files/osgModels/Gualzru/Gualzru.osg";
		mesh.meshPath = "/home/robocomp/robocomp/components/robocomp-araceli/models/human03.3ds";
		innermodelmanager_proxy->addMesh("fakeperson_mesh3", "fakeperson3", mesh);
	}
	catch (...)
	{
...
void SpecificWorker::includeInAGM()
{
	...
	while ((personSymbolId = worldModel->getIdentifierByType("person3", idx++)) != -1)
	{
		printf("%d %d\n", idx, personSymbolId);
		if (idx > 4) exit(0);
		if (worldModel->getSymbolByIdentifier(personSymbolId)->getAttribute("imName") == "fakeperson3")
		{
			printf("found %d!!\n", personSymbolId);
			break;
		}
	...
	// Symbolic part
	AGMModelSymbol::SPtr person3 =   newModel->newSymbol("person3");
	personSymbolId = person3->identifier;
	printf("Got personSymbolId: %d\n", personSymbolId);
	person3->setAttribute("imName", "fakeperson3");
	person3->setAttribute("imType", "transform");
	AGMModelSymbol::SPtr personSt = newModel->newSymbol("personSt");
	printf("person %d status %d\n", person3->identifier, personSt->identifier);

	newModel->addEdge(person3, personSt, "hasStatus");
	newModel->addEdge(person3, personSt, "noReach");
	newModel->addEdge(person3, personSt, "person");
	
	newModel->addEdgeByIdentifiers(person3->identifier, 3, "in");
	...
	edgeRTAtrs["ry"] = "3.14";
	edgeRTAtrs["rz"] = "0";
	newModel->addEdgeByIdentifiers(100, person3->identifier, "RT", edgeRTAtrs);


	AGMModelSymbol::SPtr personMesh = newModel->newSymbol("mesh");
	printf("personMesh %d\n", personMesh->identifier);
	personMesh->setAttribute("collidable", "false");
	personMesh->setAttribute("imName", "fakepersonMesh3");
	personMesh->setAttribute("imType", "mesh");
	personMesh->setAttribute("path", "/home/robocomp/robocomp/components/robocomp-araceli/models/human03.3ds");
	personMesh->setAttribute("render", "NormalRendering");
	personMesh->setAttribute("scalex", "8");
	personMesh->setAttribute("scaley", "8");
	personMesh->setAttribute("scalez", "8");
	...
	edgeRTAtrs["ry"] = "0";
	edgeRTAtrs["rz"] = "1.57079632679";
	newModel->addEdge(person3, personMesh, "RT", edgeRTAtrs);
...
void SpecificWorker::move (){
  ...
  RoboCompInnerModelManager::coord3D coordInBase;
		...
		innermodelmanager_proxy->transform("root", "fakeperson3", coordInItem, coordInBase);
		...
		innermodelmanager_proxy->setPoseFromParent("fakeperson3", pose);
