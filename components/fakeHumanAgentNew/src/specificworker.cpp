...
void SpecificWorker::includeInRCIS()
{
	...
	{	
		...
		innermodelmanager_proxy->addMesh("fakeperson_mesh1", "fakeperson1", mesh);
	}
	catch (...)
	{
...
void SpecificWorker::includeInAGM()
{
	...
	newModel->addEdge(person1, personSt, "hasStatus");
	newModel->addEdge(person1, personSt, "noReach");
	newModel->addEdge(person1, personSt, "person");
	
	newModel->addEdgeByIdentifiers(person1->identifier, 3, "in");
	...
	edgeRTAtrs["ry"] = "0";
	edgeRTAtrs["rz"] = "0";
	newModel->addEdgeByIdentifiers(100, person1->identifier, "RT", edgeRTAtrs);
	...
	newModel->addEdge(person1, personMesh, "RT", edgeRTAtrs);
