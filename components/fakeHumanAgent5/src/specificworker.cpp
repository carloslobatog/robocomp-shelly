...
void SpecificWorker::includeInAGM()
{
	...
	int idx=0;
	while ((personSymbolId = worldModel->getIdentifierByType("person5", idx++)) != -1)
	{
		printf("%d %d\n", idx, personSymbolId);
		if (idx > 4) exit(0);
		if (worldModel->getSymbolByIdentifier(personSymbolId)->getAttribute("imName") == "fakeperson5")
		{
			printf("found %d!!\n", personSymbolId);
			break;
		}
	...
	// Symbolic part
	AGMModelSymbol::SPtr person5 =   newModel->newSymbol("person5");
	personSymbolId = person5->identifier;
	printf("Got personSymbolId: %d\n", personSymbolId);
	person5->setAttribute("imName", "fakeperson5");
	person5->setAttribute("imType", "transform");
	AGMModelSymbol::SPtr personSt = newModel->newSymbol("personSt");
	printf("person %d status %d\n", person5->identifier, personSt->identifier);

	newModel->addEdge(person5, personSt, "hasStatus");
	newModel->addEdge(person5, personSt, "noReach");
	newModel->addEdge(person5, personSt, "person");
	
	newModel->addEdgeByIdentifiers(person5->identifier, 3, "in");
...
void SpecificWorker::move (){
  ...
  RoboCompInnerModelManager::coord3D coordInBase;
		...
		innermodelmanager_proxy->transform("root", "fakeperson5", coordInItem, coordInBase);
		...
		innermodelmanager_proxy->setPoseFromParent("fakeperson5", pose);
